...
import { getWorkoutPlan, getNextWorkoutTemplate } from '../data/workout-templates';
...
export function generateNextWorkout(
  ...
  const exercises: WorkoutExercise[] = template.exercises.map(templateExercise => {
    ...
    const progression = calculateProgression(
      templateExercise.exerciseId, exerciseType, lastWeight, templateExercise.reps, recentSessions,
    );
    ...
    const workoutSets: WorkoutSet[] = Array.from({ length: sets }, (_, i) => ({
      setNumber: i + 1, targetReps: reps, targetWeight, completed: false,
    }));
